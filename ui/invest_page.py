...
from ui.styles import ACTION_COLOR, SHARED_CSS

st.markdown(SHARED_CSS, unsafe_allow_html=True)
st.title("💼 투자 의사결정")
st.caption("기술적 분석 + 수급 + ML 예측 + 포트폴리오")
    ...
    if "brief" in st.session_state:
        ...
        if recs:
            ...
            for r in recs:
                ...
                st.markdown(f"**<span style='color:{color}'>{r['action']}</span>** — {r['name']} ({r['code']}) · {r['score']}점", unsafe_allow_html=True)
                o = r["order"]
                if o["quantity"]:
                    st.markdown(f"{o['quantity']}주 / ₩{o['amount']:,.0f} / 손절 ₩{o['stop_loss']:,.0f}")
                for reason in r["reasons"]:
                    st.markdown(f"- {reason}")
        else:
            st.info("적극 매수 아이디어가 없습니다.")

        if brief.portfolio_actions:
            st.subheader("보유 종목 액션")
            for a in brief.portfolio_actions:
                st.markdown(f"**{a['name']}**: {a['action']} ({a['pnl_pct']:+.1f}%)")
                if a.get("alert"):
                    st.warning(a["alert"])
    else:
        st.info("'브리핑 생성'을 눌러주세요.")
...
with tab_analyze:
    ...
    pv = st.number_input("포트폴리오 총액", value=int(portfolio_value), step=1_000_000)

    if st.button("분석 실행", type="primary", key="invest_analyze"):
        with st.spinner("분석 중..."):
            try:
    ...
    if "report" in st.session_state:
        ...
        with col_l:
            st.markdown(f"- 진입 ₩{r.entry_price:,.0f} · 손절 ₩{r.stop_loss:,.0f} · 목표 ₩{r.target_price:,.0f}")
            st.markdown(f"- 비중 {r.position_pct}%")
            if order["quantity"]:
                st.markdown(f"- 매수 제안 {order['quantity']}주 (₩{order['amount']:,.0f})")
        with col_r:
            for k, v in r.flow_5d.items():
                st.markdown(f"- {k}: {v:+,}주")
            st.markdown(f"- 변동성 {r.volatility_20d}% · RSI {r.rsi:.0f}")
        for reason in r.reasons:
            st.markdown(f"- {reason}")
        for c in r.cautions:
            st.warning(c)
...
with tab_portfolio:
    ...
    if summary.holdings:
        st.dataframe([{
            ...
            "판단": h.action,
        } for h in summary.holdings], hide_index=True, use_container_width=True)
    ...
    with e1:
        ...
        if st.button("추가", key="pf_add"):
            ...
            st.rerun()
    with e2:
        st.markdown("**종목 매도 / 현금**")
        sell_code = st.text_input("매도 코드", key="sell_code")
        sell_qty = st.number_input("매도 수량 (0=전량)", 0, step=1, key="sell_qty")
        cash_amount = st.number_input("현금", int(summary.cash), step=1_000_000, key="cash_amt")
        if st.button("매도", key="pf_sell"):
            remove_holding(sell_code, sell_qty or None)
            st.rerun()
        if st.button("현금 저장", key="pf_cash"):
            set_cash(cash_amount)
            st.rerun()

st.warning("⚠️ 투자 참고용이며, 모든 결정과 손실은 본인 책임입니다.")