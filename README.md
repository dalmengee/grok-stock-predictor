...
```bash
python backtest.py run                              # KOSPI200 이중전략 10년 백테스트
python backtest.py validate                         # 워크포워드 OOS 검증
python backtest.py compare                          # 단순 vs 이중 전략 비교
```

**유니버스**: KOSPI200 시총 상위 200종목 중 **삼성전자(005930)·SK하이닉스(000660) 제외**  
**벤치마크**: KOSPI200 **동일가중** 지수 (DB 일봉으로 합성)

| 국면 | 전략 | 주식 비중 | 현금 | 최대 종목 | 종목당 비중 |
|------|------|-----------|------|-----------|-------------|
| 상승장 | 모멘텀 동일비중 | 80% | 20% | 20 | 4% |
| 횡보장 | 선별 동일비중 | 55% | 45% | 10 | 5.5% |
| 하락장 | 현금 대기 | 0% | 100% | 0 | — |
| 위기 | 전량 현금 | 0% | 100% | 0 | — |

리밸런싱 21일(월간) · DD 스케일링: -5%→×0.75, -9%→×0.5, -13%→현금
...
streamlit run app.py                     # 통합 웹 UI (위 페이지 포함)
```
